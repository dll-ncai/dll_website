...
if ( ! function_exists( 'creativ_business_customize_features_widget_fields' ) ) :
	 ...
	 */
	function creativ_business_customize_features_widget_fields( $form_options, $widget ) {
...
add_filter( 'siteorigin_widgets_form_options_sow-features', 'creativ_business_customize_features_widget_fields', 10, 2 );

if ( ! function_exists( 'creativ_business_add_tab_in_builer_widgets_panel' ) ) :
	 ...
	 */
	function creativ_business_add_tab_in_builer_widgets_panel( $tabs ) {
		$tabs['creativ-business'] = array(
			'title'  => __( 'Creativ Business Widgets', 'creativ-business' ),
			'filter' => array(
				'groups' => array( 'creativ-business' ),
			),
		);
		return $tabs;
	}
...
add_filter( 'siteorigin_panels_widget_dialog_tabs', 'creativ_business_add_tab_in_builer_widgets_panel' );

if ( ! function_exists( 'creativ_business_group_theme_widgets_in_builder' ) ) :
	 ...
	 */
	function creativ_business_group_theme_widgets_in_builder( $widgets ) {
		...
		if ( isset( $GLOBALS['wp_widget_factory'] ) && ! empty( $GLOBALS['wp_widget_factory']->widgets ) ) {
			...
			foreach ( $all_widgets as $widget ) {
				if ( false !== strpos( $widget, 'Creativ_Business_' ) ) {
					$widgets[ $widget ]['groups'] = array( 'creativ-business' );
...
add_filter( 'siteorigin_panels_widgets', 'creativ_business_group_theme_widgets_in_builder' );


if ( ! function_exists( 'creativ_business_panels_row_style_attributes' ) ) :
	 ...
	 */
	function creativ_business_panels_row_style_attributes( $attr, $style ) {
...
add_filter( 'siteorigin_panels_row_style_attributes', 'creativ_business_panels_row_style_attributes', 10, 2 );

if ( ! function_exists( 'creativ_business_customize_so_widgets_status' ) ) :
	 ...
	 */
	function creativ_business_customize_so_widgets_status( $active ) {
...
add_filter( 'siteorigin_widgets_active_widgets', 'creativ_business_customize_so_widgets_status' );
