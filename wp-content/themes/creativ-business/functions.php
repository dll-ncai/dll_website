<?php
/**
 * Creativ Business functions and definitions.
 *
 * @package Creativ Business
 */

if ( ! function_exists( 'creativ_business_setup' ) ) :
	/**
	 ...
	 */
	function creativ_business_setup() {
		/*
		 * Make theme available for translation.
		 */
		load_theme_textdomain( 'creativ-business' );
		...
		add_image_size( 'creativ-business-featured-banner', 1400, 320, true );

		// This theme uses wp_nav_menu() in four location.
		register_nav_menus( array(
			'primary'  => esc_html__( 'Primary Menu', 'creativ-business' ),
		) );
		...
		add_theme_support( 'custom-background', apply_filters( 'creativ_business_custom_background_args', array(
			'default-color' => '#ffffff',
			'default-image' => '',
		) ) );

		// Set up the WordPress core custom header feature.
		add_theme_support( 'custom-header', apply_filters( 'creativ_business_custom_header_args', array(
		...
		$creativ_business_default_options = creativ_business_get_default_theme_options();
...
function creativ_business_fonts_url() {
	...
	if ( 'off' !== _x( 'on', 'Montserrat: on or off', 'creativ-business' ) ) {
 ...
 */
function creativ_business_scripts() {

	$min = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';

	$fonts_url = creativ_business_fonts_url();
	if ( ! empty( $fonts_url ) ) {
		wp_enqueue_style( 'creativ-business-google-fonts', $fonts_url, array(), null );
	}
	...
	wp_enqueue_style( 'creativ-business-style', get_stylesheet_uri(), array(), '1.4.0' );

	wp_enqueue_script( 'creativ-business-skip-link-focus-fix', get_template_directory_uri() . '/assets/js/skip-link-focus-fix' . $min . '.js', array(), '1.2.0', true );
	...
	wp_enqueue_script( 'creativ-business-custom', get_template_directory_uri() . '/assets/js/custom' . $min . '.js', array( 'jquery' ), '1.2.0', true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'creativ_business_scripts' );
...
function creativ_business_add_menu_parent_class( $items ) {
    $parents = array();
...
if ( ! function_exists( 'creativ_business_excerpt_length' ) ) :
	/**
	 * excerpt length.
	 *
	 ...
	 *
	 */
	function creativ_business_excerpt_length( $length ) {
		if ( is_admin() ) {
			return $length;
		}

		$length = creativ_business_get_option( 'excerpt_length' );
		$length = ! empty( $length ) ? $length : '25';
		return $length;
	}
endif;
add_filter( 'excerpt_length', 'creativ_business_excerpt_length', 999 );

if( ! function_exists( 'creativ_business_excerpt_more' ) ) :
	/**
	 * @param string $more.
	 *
	 * Return string 
	 */
	function creativ_business_excerpt_more( $more ) {
		if ( is_admin() ) {
			return $more;
		}
		/* translators: %s: post title */
		$more =	sprintf( __( '<span class="screen-reader-text"> "%s"</span>', 'creativ-business' ), get_the_title( get_the_ID() ) );
		return ' &hellip; ' . $more;
	}
endif;	
add_filter('excerpt_more', 'creativ_business_excerpt_more');
...
if ( ! function_exists( 'creativ_business_get_sidebar_options' ) ) :

	function creativ_business_get_sidebar_options() {
...
if ( ! function_exists( 'creativ_business_get_index_page_id' ) ) :
	 ...
	 */
	function creativ_business_get_index_page_id( $type = 'front' ) {
...
if ( ! function_exists( 'creativ_business_content_is_pagebuilder' ) ) :
...
add_action( 'tgmpa_register', 'creativ_business_activate_recommended_plugins' );

/**
 * Register recommended plugins.
 */

function creativ_business_activate_recommended_plugins() {

	$plugins = array(

		array(
			'name'     => __( 'Page Builder by SiteOrigin', 'creativ-business' ),
			'slug'     => 'siteorigin-panels',
			'required' => false,
		),
		array(
			'name'     => __( 'SiteOrigin Widgets Bundle', 'creativ-business' ),
			'slug'     => 'so-widgets-bundle',
			'required' => false,
		),
		array(
			'name'     => __( 'Contact Form 7', 'creativ-business' ),
			'slug'     => 'contact-form-7',
			'required' => false,
		),
	);
